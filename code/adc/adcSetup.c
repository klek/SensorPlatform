...
void adcSetup(ADC_HandleTypeDef* adcA, ADC_HandleTypeDef* adcB)
{
	...
	adcA->Init.ContinuousConvMode    = DISABLE;//ENABLE;                       	// Continuous mode enabled to have continuous conversion
	adcA->Init.DiscontinuousConvMode = DISABLE;                       	// Parameter discarded because sequencer is disabled
	adcA->Init.NbrOfDiscConversion   = 0;
	adcA->Init.ExternalTrigConvEdge  = ADC_EXTERNALTRIGCONVEDGE_RISING;//ADC_EXTERNALTRIGCONVEDGE_NONE;	// Conversion start trigged at each external event
	adcA->Init.ExternalTrigConv      = ADC_EXTERNALTRIGCONV_T2_TRGO;//ADC_EXTERNALTRIGCONV_T1_CC1;
	...
	adcB->Init.ContinuousConvMode    = DISABLE;//ENABLE;                       	// Continuous mode enabled to have continuous conversion
	adcB->Init.DiscontinuousConvMode = DISABLE;                       	// Parameter discarded because sequencer is disabled
	adcB->Init.NbrOfDiscConversion   = 0;
	adcB->Init.ExternalTrigConvEdge  = ADC_EXTERNALTRIGCONVEDGE_RISING;//ADC_EXTERNALTRIGCONVEDGE_NONE;   // Conversion start trigged at each external event
	adcB->Init.ExternalTrigConv      = ADC_EXTERNALTRIGCONV_T2_TRGO;//ADC_EXTERNALTRIGCONV_T1_CC1;
