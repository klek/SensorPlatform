...
for n = 1:size(index,1)
  ...
  if isImage
    %set(get(hDatatip(n),'Cursor'),'DataIndex',pos,...
      %'Position',pos(1:2))
      % MODIFIED TO GET RID OF ERROR
      hDatatipCursor = get(hDatatip(n), 'Cursor');
      hDatatipCursor.DataIndex = pos;
      hDatatipCursor.Position = pos(1:2);
  else
    %set(get(hDatatip(n),'Cursor'),'DataIndex',index(n, :),...
      %'Position',pos)
      % MODIFIED TO GET RID OF ERROR
      hDatatipCursor = get(hDatatip(n), 'Cursor');
      hDatatipCursor.DataIndex = index(n, :);
      hDatatipCursor.Position = pos;
  end
